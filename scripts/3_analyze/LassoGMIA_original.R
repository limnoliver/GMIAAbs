...
# Load data
load("dataMergeFinal.RData")

################################################
# Rearrange dataframe so all IVs are at the end#
2222
extras <- names(dataMerge)[c(1:19,174,175)]
IVs <- multiGrep2(c("A","Resids","Sag"),names(dataMerge),ignore.case = FALSE)
IVs <- IVs[-grep("Acetate",IVs)]
extras <- names(dataMerge)[-which(names(dataMerge) %in% IVs)]
df.orig <- dataMerge[,c(extras,IVs)]
################################################

#######################################
#log transform absorbance coefficients#
zeros <- numeric()
AbsVars <- grep("A",IVs,value=TRUE)

#find min non-zero and assign values of zero or less 1/2 that value
for(var in AbsVars){
  negVals <- which(df.orig[,var]<=0)
  if(length(negVals) > 0){
    replaceNeg <- min(df.orig[-negVals,var])/2
    df.orig[negVals,var] <- replaceNeg
  }
}
...
# set the first independent variable in the dataframe. All variables after this will be assumed
# to be IVs as well
beginIV <- "A698"
...
logIVs <- grep("A",IVs,value = TRUE) #which IVs should be log transformed
...
df <- df.orig
df$colors <- as.numeric(as.factor(df$Site))
df <- df[,c(response,"colors",IVs)]

df[,logIVs] <- log10(df[,logIVs])

if(logResponse==TRUE){df[,response] <- log10(df[,response])} #log transform response variable
...
pdf(paste(parameter,filetype,"CVgraphsAlass.pdf",sep=""))
CV.dev <- dev.cur()
...
############ Graphics ###########################
pdf(paste(parameter,filetype,"ModelgraphsCenAndStepAlass.pdf",sep=""))
model.dev <- dev.cur()
...
if(vars.1se.exists){
  ...
  #Rsquared <- rSqr(observed = y,resid = (predictions.step-y),p = length(vars.1se))
  
  dfe <- length(y)-length(vars.1se)-1
  dft <- length(y)-1
  SSres <- sum((y-predictions[,1])^2)
  SStot <- sum((y-mean(y))^2)
  Rsquared <- 1-((SSres/dfe)/(SStot/dft))
  
  mtext(paste("aR2 =",round(Rsquared,3)),side=3,line=-1)
...
if(length(testvars.orig.1se)>0){
  ...
  mtext(paste(method,": ",response,sep=""),line=0.,side=3,font=2)
  #Rsquared <- rSqr(observed = y,resid = (predictions.step-y),p = length(vars.CVcen))
  
  dfe <- length(y)-length(vars.CVcen)-1
  dft <- length(y)-1
  SSres <- sum((y-predictions[,1])^2)
  SStot <- sum((y-mean(y))^2)
  Rsquared <- 1-((SSres/dfe)/(SStot/dft))
...
if(length(testvars)<40 & length(testvars)>0 & length(coef(m2))>1){
  ...
  mtext(paste(method,": ",response,sep=""),line=0.,side=3,font=2)
  #Rsquared <- rSqr(observed = y,resid = (predictions.step-y),p = length(modelvars))
  
  dfe <- length(y)-length(modelvars)-1
  dft <- length(y)-1
  SSres <- sum((y-predictions[,1])^2)
  SStot <- sum((y-mean(y))^2)
  Rsquared <- 1-((SSres/dfe)/(SStot/dft))
...
if(length(testvars)<40 & length(testvars)>0 & length(coef(m2.step))>1){
  ...
  #Rsquared <- rSqr(observed = y,resid = (predictions.step-y),p = length(modelvars.step))
  
  dfe <- length(y)-length(modelvars.step)-1
  dft <- length(y)-1
  SSres <- sum((y-predictions[,1])^2)
  SStot <- sum((y-mean(y))^2)
  Rsquared <- 1-((SSres/dfe)/(SStot/dft))
...
#  shell.exec(paste(parameter,"ModelgraphsCenAndStep.pdf",sep=""))
